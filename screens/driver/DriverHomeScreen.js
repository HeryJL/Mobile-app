...
  }, []); // Empty dependency array means this runs once on mount
  ...
  const simulateNewTrip = async () => {
      if (isAvailable && !hasOngoingTrip) {
          setLoading(true); // Show main loading indicator while simulating trip acceptance
          console.log("Simulating receiving a new trip...");
           await new Promise(resolve => setTimeout(resolve, 1500)); // Simulate network delay

          setHasOngoingTrip(true);
          setOngoingTripDetails(dummyTrip); // Set dummy trip data
          setIsAvailable(false); // Driver becomes unavailable once on a trip

          // Fetch routes for the new trip
          const current = driverLocation || await getDriverLocation(); // Use current location or fetch it
          if (current) {
              const pathToPickup = await getRoute(current, dummyTrip.pickupCoords);
              const pathToDropoff = await getRoute(dummyTrip.pickupCoords, dummyTrip.dropoffCoords);
              setRouteToPickup(pathToPickup);
          ...
          setLoading(false);
          Alert.alert("Nouvelle course!", "Vous avez une nouvelle course attribuée.");
      } else if (hasOngoingTrip) {
           Alert.alert("Occupé", "Vous avez déjà une course en cours.");
      } else {
           Alert.alert("Indisponible", "Mettez-vous en 'Disponible' pour recevoir des courses.");
      }
  };
...
        )}
        ...
        {/* Simulate receiving a new trip button (for testing) */}
        {!hasOngoingTrip && isAvailable && !loadingAvailability && (
             <TouchableOpacity style={styles.simulateButton} onPress={simulateNewTrip}>
                <Text style={styles.simulateButtonText}>Simuler nouvelle course</Text>
             </TouchableOpacity>
        )}
